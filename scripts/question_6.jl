...
file_path = joinpath(@__DIR__, "instances/KIRO-tiny_20.json")
...
max_rating_S_Q⁰ = maximum([max_rating_S, max_rating_Q⁰])
Cⁿ = [sum(linearize_positive_part(Ω[ω]["power_generation"] * sum(z[v, t] for t ∈ indices_Vᵗ) - linearize_min(sum(S[s]["rating"] * x[v, s] for s ∈ indices_S), sum(Q⁰[q]["rating"] * y⁰[v, q] for q ∈ indices_Q⁰), max_rating_S_Q⁰), maximum([Ω[ω]["power_generation"]*length(Vᵗ), minimum([max_rating_Q⁰, max_rating_S])])) for v ∈ indices_Vˢ) for ω ∈ indices_Ω]

# Construction de Cᶠ
max_rating_Qˢ = maximum([Qˢ[q]["rating"] for q ∈ indices_Qˢ])
Cᶠ = [linearize_positive_part(Ω[ω]["power_generation"] * sum(z[v, t] for t ∈ indices_Vᵗ) - sum(Qˢ[q]["rating"] * yˢ[ẽ, q] for ẽ ∈ indices_Eˢ, q ∈ indices_Qˢ if v ∈ Eˢ[ẽ]), Ω[ω]["power_generation"]*length(Vᵗ)+max_rating_Qˢ) +  
      sum(linearize_positive_part(Ω[ω]["power_generation"] * sum(z[(Eˢ[ẽ][1]!=v ? Eˢ[ẽ][1] : Eˢ[ẽ][2]), t] for t ∈ indices_Vᵗ) + linearize_min(sum(Qˢ[q]["rating"]*yˢ[ẽ, q] for q ∈ indices_Qˢ), Ω[ω]["power_generation"] * sum(z[v, t] for t ∈ indices_Vᵗ), max_rating_Qˢ + Ω[ω]["power_generation"] * length(Vᵗ)) - linearize_min(sum(S[s]["rating"] * x[(Eˢ[ẽ][1]!=v ? Eˢ[ẽ][1] : Eˢ[ẽ][2]), s] for s ∈ indices_S), sum(Q⁰[q]["rating"] * y⁰[(Eˢ[ẽ][1]!=v ? Eˢ[ẽ][1] : Eˢ[ẽ][2]), q] for q ∈ indices_Q⁰), max_rating_S_Q⁰), maximum([Ω[ω]["power_generation"]*length(Vᵗ) + minimum([max_rating_Qˢ, Ω[ω]["power_generation"] * length(Vᵗ)]), minimum([max_rating_Q⁰, max_rating_S])])) for ẽ ∈ indices_Eˢ if v ∈ Eˢ[ẽ])
...
Cᶜ_Cⁿ = [c₀*Cⁿ[ω] + cₚ*linearize_positive_part(Cⁿ[ω]-Cₘₐₓ, maximum([Ω[ω]["power_generation"]*length(Vᵗ), Cₘₐₓ])) for ω ∈ indices_Ω]
Cᶜ_Cᶠ = [c₀*Cᶠ[v, ω] + cₚ*linearize_positive_part(Cᶠ[v, ω]-Cₘₐₓ, maximum([(Ω[ω]["power_generation"]*length(Vᵗ) + minimum([max_rating_Qˢ, Ω[ω]["power_generation"]*length(Vᵗ)])) * length(Vˢ), Cₘₐₓ])) for v ∈ indices_Vˢ, ω ∈ indices_Ω]

# Construction de pᶠ
pᶠ = [sum(S[s]["probability_of_failure"]*x[v, s] for s ∈ indices_S) + sum(Q⁰[q]["probability_of_failure"]*y⁰[v, q] for q ∈ indices_Q⁰) for v ∈ indices_Vˢ]
...
c = sum(S[s]["cost"]*x[v, s] for v ∈ indices_Vˢ, s ∈ indices_S) +
    ...
    sum(Ω[ω]["probability"] * (sum(pᶠ[v] * Cᶜ_Cᶠ[v, ω] for v ∈ indices_Vˢ) + (1 - sum(pᶠ)) * Cᶜ_Cⁿ[ω]) for ω ∈ indices_Ω)
